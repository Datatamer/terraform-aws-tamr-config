...
resource "local_file" "cloudwatch-install" {
  ...
  content  = templatefile("${path.module}/files/emr-cloudwatch-install.tpl", { region = "us-east-2", endpoint = module.vpc.vpce_logs_endpoint_dnsname, log_group = aws_cloudwatch_log_group.tamr_log_group.name })
}
